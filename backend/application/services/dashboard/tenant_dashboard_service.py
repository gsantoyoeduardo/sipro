...
class TenantDashboardService:
    ...
    def obtener_kpis(user):
        ...
        idempresa = getattr(user, 'idempresa_id', None)
        empresa = Empresa.objects.filter(idempresa=idempresa).first() if idempresa else None
        sucursales = list(Sucursal.objects.filter(estado=True).values('idsucursal', 'nombre', 'codigo', 'direccion', 'estado'))
        almacenes = list(Almacen.objects.filter(estado=True).select_related('idsucursal').values('idalmacen', 'nombre', 'codigo', 'idsucursal', 'idsucursal__nombre', 'estado'))
