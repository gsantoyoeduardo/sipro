...
const inputClass = 'w-full px-4 py-3 border border-borde rounded-lg focus:ring-2 focus:ring-primario focus:outline-none text-texto'

export default function FormularioAdmin({ form, onChange, onAtras, isSubmitting }: FormularioAdminProps) {
  return (
    <div className="space-y-4 sm:space-y-5">
      <h2 className="font-semibold text-texto border-b border-borde-suave pb-2 text-sm sm:text-base">Datos del Administrador</h2>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <button type="button" onClick={onAtras} className="flex-1 px-4 py-3 border border-borde rounded-lg hover:bg-fondo-sutil transition font-semibold">
          Atrás
        </button>
        <button type="submit" disabled={isSubmitting} className="flex-1 bg-primario text-white py-3 rounded-lg font-semibold hover:bg-primario-oscuro transition disabled:opacity-50 flex items-center justify-center gap-2">
          {isSubmitting && (
