...
const inputClass = 'w-full px-4 py-3 border border-borde rounded-lg focus:ring-2 focus:ring-accion focus:outline-none text-texto'

export default function FormularioEmpresa({ form, onChange, onContinuar }: FormularioEmpresaProps) {
  return (
    <div className="space-y-4 sm:space-y-5">
      <h2 className="font-semibold text-texto border-b border-borde-suave pb-2 text-sm sm:text-base">Datos de la Empresa</h2>
      <div>
      ...
      <button type="button" onClick={onContinuar} className="w-full bg-accion text-white py-3 rounded-lg font-semibold hover:bg-accion/90 transition">
        Continuar
      </button>
    </div>
  )
