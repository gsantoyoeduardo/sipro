...
export default function Sidebar({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed lg:static inset-y-0 left-0 z-30 bg-[#0A31A6] text-white flex flex-col transition-all duration-300 ${
          collapsed ? 'w-0 lg:w-16 overflow-hidden' : 'w-64'
        }`}
      >
        <div className="flex items-center gap-3 px-4 h-16 border-b border-white/10 shrink-0">
          <img src="/logo.png" alt="S" className="w-8 h-8 brightness-0 invert" />
          {!collapsed && <span className="font-bold text-lg">SIPRO Portal</span>}
        </div>
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition ${
                  isActive
                    ? 'bg-[#0785F2]/20 text-[#38F2F2] border-l-2 border-[#38F2F2] rounded-l-none'
                    : 'text-white/70 hover:bg-[#0F3DB8] hover:text-white'
                }`}
              >
